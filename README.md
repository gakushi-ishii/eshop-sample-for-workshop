...
答えが漏れてしまいます。これを防ぐため、preToolUse Hook（`file-access-guard`）が
エージェントのファイル操作を制御します。既定（実習モード）では次を拒否します。

- `lab/**` の参照（教材のコンテキストへの注入を防止）
- `.env` の参照・変更（モード切替ファイルそのものの保護）
- `.github/hooks/**` への書き込み（ガード定義の無効化を防止。読み取りは可能）

モードは `.env` の `LAB_GUARD_MODE` で切り替えます。Hook スクリプトが実行時に
`.env` を機械的に読むため、GitHub Copilot App が CLI をバックグラウンド起動する
場合でも、ファイルを編集するだけで即時に反映されます（App の再起動は不要）。

- `LAB_GUARD_MODE=edit` … 教材編集モード。上記ガードを全解除します。
- 未設定 または `LAB_GUARD_MODE=practice` … 通常の実習モード。従来どおりブロックします。

教材を編集したい場合は、`.env.sample` を `.env` にコピーして `LAB_GUARD_MODE=edit`
を設定します（`.env` は Git 管理外。テンプレートの `.env.sample` のみ追跡します）。

この切替は **人間** が `.env` を編集して行う運用を前提とします。エージェントは実習
モード中に `.env` や Hook を変更できないため、自分でガードを迂回できません。ただし
シェル経由の難読化された書き込みなど、文字列照合では完全に防ぎきれない経路は残ります。
教材から得た情報を本番実装へ流用しないポリシーは、モードに関わらず維持してください。
