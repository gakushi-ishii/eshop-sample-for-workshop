...
- ユーザーが教材自体の編集またはレビューを明示的に依頼した場合のみ、指定されたファイルに
  限って参照できる。教材から得た情報をアプリケーション実装へ流用しない。
- この分離は preToolUse Hook（`.github/hooks/scripts/file-access-guard.mjs`）で機械的に
  強制する。実習モード（既定）では `lab/**` の参照、`.env` の参照・変更、
  `.github/hooks/**` への書き込みを拒否する。モード切替は人間が `.env` の
  `LAB_GUARD_MODE`（`practice` / `edit`）で行い、エージェントからは切り替えられない。
